...
public class DirectoryMonitor extends Observable implements Runnable {
	...
	public void run() {
		try {
				...
				for (int i = 0; i < files.length; i++) {
					if (files[i].isFile()) {
						if (fileMap.containsKey(files[i])) {
							if (fileMap.get(files[i]).longValue() < files[i].lastModified()) {
								fileMap.put(files[i], new Long(files[i].lastModified()));
			...
			errorFile.writeEvent("DirectoryMonitor", "run", "Unhandled exception: " + t);
		}
	}
}
